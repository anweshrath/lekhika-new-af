...
class WorkflowExecutionService {
  ...
  async executeProcessNode(nodeData, pipelineData, progressCallback = null, workflowId = null) {
    ...
    // SURGICAL FIX: Map generic node IDs to proper roles for user engines
    let nodeRole = nodeData.role || nodeData.id
    
    // Map generic process nodes to proper roles based on label
    if (nodeData.label && !nodeData.role) {
      if (nodeData.label.includes('World-Building') || nodeData.label.includes('Character Development')) {
        nodeRole = 'world_builder'
      } else if (nodeData.label.includes('Plot Architecture') || nodeData.label.includes('Story Structure')) {
        nodeRole = 'plot_architect'
      } else if (nodeData.label.includes('Literary Writing') || nodeData.label.includes('Narrative Craft')) {
  ...
  async executeSingleAIGeneration(nodeData, pipelineData, progressCallback = null, workflowId = null) {
    ...
    // SURGICAL FIX: Add permission validation before AI execution
    const nodeRole = nodeData.role || nodeData.id
    const roleConfig = NODE_ROLE_CONFIG[nodeRole]
    
    if (!roleConfig) {
      console.warn(`⚠️ No role config found for node role: ${nodeRole}`)
    } else {
      console.log(`🔒 PERMISSION CHECK: Node ${nodeData.id} (${nodeData.label}) has role: ${nodeRole}`)
      console.log(`🔒 PERMISSIONS: canWriteContent=${roleConfig.canWriteContent}, canEditStructure=${roleConfig.canEditStructure}, canProofRead=${roleConfig.canProofRead}`)
      
      // Check if this node is authorized to write content
      if (!roleConfig.canWriteContent && nodeData.label?.toLowerCase().includes('writing')) {
        console.warn(`⚠️ PERMISSION VIOLATION: Node ${nodeData.id} (${nodeRole}) is not authorized to write content`)
        // Don't block execution, but log the violation for monitoring
      }
    }
...
REFINED CONTENT:`
  ...
  processPromptVariables(prompts, pipelineData, nodePermissions = null) {
    ...
    console.log(`🔍 Processed variables for node:`, {
...
Respond with ONLY a single number (e.g., "5" or "6"). No explanation needed.`
  ...
  async formatFinalOutput(compiledContent, formatOptions) {
    ...
    if (!formatsToGenerate.includes('html') && !formatsToGenerate.includes('HTML')) {
      ...
      console.log('📄 Added HTML format for preview')
    }
    ...
    for (const format of formatsToGenerate) {
      ...
      try {
        ...
        const beautifulContent = await professionalBookFormatter.formatCompleteBook(
          ...
          userInput, 
          formatStr,
          { ...typographyPrefs, rawContent: bookContent }
        )
