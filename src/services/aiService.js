...
Return ONLY the chapter content in markdown format.`;
  ...
  async callAIWithQueue(prompt, type, maxRetries = 3) {
    // NO RETRIES OR FALLBACKS - USE FIRST PROVIDER ONLY
    const provider = this.getNextProvider();
    
    try {
      console.log(`🚀 Using ${provider.toUpperCase()} for ${type} - NO FALLBACKS`);
      ...
      throw new Error(`AI Provider ${provider.toUpperCase()} failed: ${error.message}. NO FALLBACKS.`);
    }
  }
  ...
  getEndpoint(providerType, apiKey = null) {
    switch (providerType) {
      ...
      case 'gemini':
        ...
        return `/api/gemini/models/gemini-pro:generateContent?key=${apiKey}`;
      case 'anthropic':
