...
class InMemoryQueueAdapter extends QueueAdapter {
  ...
  async clearWaiting() {
    ...
    this.pending = this.pending.filter(job => job.state !== 'waiting');
    logger.info(`InMemoryQueue: cleared ${cleared} waiting jobs`);
    return cleared;
  }
  ...
  async resetAll() {
    let cleared = 0;
    this.jobs.forEach((job, id) => {
      if (job.state === 'waiting' || job.state === 'failed' || job.state === 'active') {
        this.jobs.delete(id);
        cleared += 1;
      }
    });
    this.pending = [];
    logger.warn(`InMemoryQueue: HARD RESET applied – removed ${cleared} waiting/failed/active jobs`);
    return {
      cleared
    };
  }
