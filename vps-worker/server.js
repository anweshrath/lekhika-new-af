...
// Mount orchestration and system routers
app.use('/orchestration', orchestrationRouter);
app.use('/system', systemRouter);
logger.info('✅ Orchestration API router mounted at /orchestration');
logger.info('✅ System Control API router mounted at /system');
  ...
  try {
    ...
    try {
      ...
      if (queueEnabled && queue.connection) {
        ...
        logger.info('✅ Worker Registry initialized - worker registered for auto-discovery');
        ...
        logger.info('🎛️ Orchestration Control Center ACTIVE');
      } else {
        logger.info('⚠️ Orchestration features disabled (QUEUE_ENABLED=false or no Redis)');
      }
    } catch (orchestrationError) {
      logger.warn('⚠️ Orchestration services initialization failed:', orchestrationError.message);
      logger.warn('Core worker features will continue, orchestration features disabled');
    }
