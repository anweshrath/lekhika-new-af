-- SURGICAL FIX: Add missing category column if table exists without it
-- Boss: Fix book_templates table creation issue

-- First, check if table exists and add missing column if needed
DO $$ 
BEGIN
    IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'book_templates') THEN
        -- Table exists, check if category column is missing
        IF NOT EXISTS (SELECT FROM information_schema.columns 
                      WHERE table_name = 'book_templates' AND column_name = 'category') THEN
            ALTER TABLE book_templates ADD COLUMN category VARCHAR(100);
            RAISE NOTICE 'Added category column to existing book_templates table';
        END IF;
    ELSE
        ...
        RAISE NOTICE 'Created book_templates table';
    END IF;
END $$;

-- Now insert templates (safe even if they exist)
...
-- SUCCESS: book_templates table fixed and populated with 10 templates
